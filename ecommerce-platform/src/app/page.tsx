...
// This means Google can index the products, and users see content instantly
...
async function getFeaturedProducts(): Promise<Product[]> {
  try {
    const res = await fetch(
      ...
      { next: { revalidate: 1800 } } // ISR — rebuild every 30 minutes
    )
    if (!res.ok) return []
    ...
    <main className="min-h-screen bg-slate-50">
      ...
      <section className="bg-gradient-to-br from-emerald-600 to-emerald-800 text-white py-20 px-6">
        <div className="max-w-6xl mx-auto text-center">
          <h1 className="text-5xl font-bold mb-4 tracking-tight">Welcome to ShopHub</h1>
            ...
            {categories.map((cat: { id: string; name: string; slug: string }) => (
              <Link key={cat.id} href={`/products?category=${cat.slug}`}
                ...
                <span className="text-2xl">
                  {cat.slug === 'laptops' ? '💻' : cat.slug === 'phones' ? '📱' :
                   cat.slug === 'audio' ? '🎧' : cat.slug === 'tablets' ? '📱' :
                   cat.slug === 'wearables' ? '⌚' : '🖥️'}
                </span>
      ...
      <section className="max-w-6xl mx-auto px-6 py-8 pb-16">
        <div className="flex items-center justify-between mb-8">
          <div>
            ...
            <p className="text-slate-500 mt-1">Handpicked for you today</p>
          </div>
